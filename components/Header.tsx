...
import { ShoppingCart, Search, User, Menu, Settings, X, ChevronDown, ChevronRight, Sparkles, ArrowRight } from 'lucide-react';
import { Page, Product } from '../types';
import { createSlug } from '../utils';

interface HeaderProps {
  cartCount: number;
  onNavigate: (page: Page, categorySlug?: string) => void;
  currentPage: Page;
  onOpenCart: () => void;
  dynamicMegaMenu: any[];
}
      ...
      <div className="fixed top-0 left-0 right-0 z-[100] transition-all duration-300">
        ...
        <div className={`bg-slate-900 text-white py-2 text-center text-[10px] font-black uppercase tracking-[0.2em] transition-all ${isScrolled ? 'h-0 opacity-0 overflow-hidden' : 'h-auto opacity-100'}`}>
          <span className="flex items-center justify-center gap-2">
            ...
            ΔΩΡΕΑΝ ΠΑΡΑΔΟΣΗ ΣΤΗΝ ΗΠΕΙΡΩΤΙΚΗ ΕΛΛΑΔΑ • 20% ΕΚΠΤΩΣΗ ΓΙΑ ΝΕΟΥΣ ΠΕΛΑΤΕΣ
          </span>
        </div>
        ...
        <header className={`transition-all duration-300 ${isScrolled ? 'bg-white/95 backdrop-blur-md py-2 shadow-xl' : 'bg-white py-6 border-b border-slate-100'}`}>
          <div className="container mx-auto px-4 flex items-center justify-between gap-6">
            ...
            <div className="hidden lg:flex flex-grow max-w-2xl px-8">
              <form onSubmit={handleSearch} className="flex items-center bg-slate-50 border border-slate-200 rounded-lg px-5 py-2.5 w-full focus-within:border-indigo-400 focus-within:bg-white transition-all shadow-sm">
                <input 
                  type="text" 
                  placeholder="Αναζήτηση σε 5000+ προϊόντα..." 
                  className="bg-transparent border-none outline-none w-full text-sm font-medium text-slate-600"
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                />
                <button type="submit" className="p-1 text-slate-400 hover:text-indigo-600 transition-colors">
                  <Search size={20} />
                </button>
              </form>
            </div>
              ...
              {dynamicMegaMenu.map((item) => (
                ...
                >
                  <button 
                    ...
                    onClick={() => onNavigate(Page.Shop, createSlug(item.title))}
                  >
                    {item.title}
                  </button>
                  ...
                  {item.subCategories.length > 0 && (
                    ...
                    >
                      <div className="container mx-auto px-4 py-12 flex gap-12 text-left">
                        {/* Subcategories Grid */}
                        <div className="flex-grow grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-x-8 gap-y-12">
                          {item.subCategories.map((sub: any) => (
                            <div 
                              ...
                              onClick={() => { 
                                const parentSlug = createSlug(item.title);
                                const childSlug = createSlug(sub.name);
                                onNavigate(Page.Shop, `${parentSlug}/${childSlug}`); 
                                setActiveMenu(null); 
                              }}
                            >
                        ...
                        {/* Side Preview */}
                        {item.featuredImage && (
                          <div className="w-72 shrink-0 relative rounded-[32px] overflow-hidden group/feat cursor-pointer shadow-lg border border-slate-100 h-80" onClick={() => onNavigate(Page.Shop, createSlug(item.title))}>
